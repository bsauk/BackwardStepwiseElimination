...
/***************************************************************************//**
    Purpose
    -------
    DGEQRF2 computes a QR factorization of a real M-by-N matrix A:
    A = Q * R.
    ...
    dA_array Array of pointers, dimension (batchCount).
             Each is a DOUBLE PRECISION array on the GPU, dimension (LDDA,N+NRHS)
             On entry, the M-by-N+NRHS matrix A. Where the last NRHS columns are B.
...
#define dA(i, j)  (dA + (i) + (j)*ldda)
  ...
  magma_dmalloc(&dwork,  (2 * nb * (n+nrhs)) * batchCount);
  ...
  magmablas_dlaset( MagmaFull, 2*nb, (n+nrhs)*batchCount, MAGMA_D_ZERO, MAGMA_D_ZERO, dwork, 2*nb, queue ); 
  ...
  for (i=0; i < min_mn; i += nb)
    {
	  ...
	  //reset dW4_displ
	  ldw = nb;
	  magma_dset_pointer( dW4_displ, dwork, 1, 0, 0,  ldw*(n+nrhs), batchCount, queue );
	  offset = ldw*n*batchCount;
	  magma_dset_pointer( dW5_displ, dwork + offset, 1, 0, 0,  ldw*(n+nrhs), batchCount, queue );    
	  ...
	  //          USE STREAM  GEMM
	  //-------------------------------------------
	  use_stream = magma_drecommend_cublas_gemm_stream(MagmaNoTrans, MagmaNoTrans, m-i-ib, n+nrhs-i-ib, ib);
	  ...
	  if ( use_stream )   
		  ...
		  magma_dlarfb_gpu_gemm( MagmaLeft, MagmaConjTrans, MagmaForward, MagmaColumnwise,
					 ...
					 dwork + nb * (n+nrhs) * k, -1,
					 dwork + nb * (n+nrhs) * batchCount + nb * (n+nrhs) * k, -1, queues[streamid] );
                }
